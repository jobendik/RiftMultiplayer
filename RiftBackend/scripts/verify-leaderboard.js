...
async function verifyLeaderboard() {
    ...
    if (leaderboard.length > 0) {
        ...
        if (topPlayer.rank !== 1)
            console.error('❌ Top player rank is not 1');
        if (!topPlayer.username)
            console.error('❌ Top player missing username');
        if (typeof topPlayer.xp !== 'number')
            console.error('❌ Top player missing XP');
        // Check sorting
        let sorted = true;
        ...
        if (sorted)
            console.log('✅ Leaderboard is sorted by XP descending');
    }
    else {
...
//# sourceMappingURL=verify-leaderboard.js.map