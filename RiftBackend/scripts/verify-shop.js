...
async function verifyShop() {
    ...
    if (!itemsRes.ok)
        throw new Error(`Fetch items failed: ${itemsRes.status}`);
    const items = await itemsRes.json();
    console.log(`✅ Fetched ${items.length} items`);
    if (items.length === 0)
        throw new Error('No items in shop');
    ...
    if (!invRes.ok)
        throw new Error(`Fetch inventory failed: ${invRes.status}`);
    const inventory = await invRes.json();
    console.log('Current Inventory:', inventory);
    const owned = inventory.find((i) => i.itemId === itemToBuy.id);
    if (owned) {
...
//# sourceMappingURL=verify-shop.js.map