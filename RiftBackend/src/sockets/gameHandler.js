...
exports.setupGameHandler = void 0;
const prisma_1 = __importDefault(require("../prisma")); // Import prisma for persistence
// Map<matchId, MatchState>
const matches = new Map();
const setupGameHandler = (io, socket) => {
...
async function endMatch(io, matchId) {
    const match = matches.get(matchId);
    if (!match)
        return;
    ...
    for (const [pId, stats] of Object.entries(match.players)) {
        ...
        try {
            // Update User Stats
            await prisma_1.default.stats.upsert({
