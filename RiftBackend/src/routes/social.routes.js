...
router.get('/friends', checkAuth, async (req, res) => {
    ...
    try {
        ...
        const friends = userWithFriends.friends.map((f) => ({
            id: f.friend.id,
...
router.get('/friend-requests', checkAuth, async (req, res) => {
    const userId = req.userId;
    try {
        const incoming = await prisma_1.default.friendRequest.findMany({
            where: { receiverId: userId, status: 'PENDING' },
            include: { sender: true }
        });
        const outgoing = await prisma_1.default.friendRequest.findMany({
            where: { senderId: userId, status: 'PENDING' },
            include: { receiver: true }
        });
        const incomingDetails = incoming.map((r) => ({
            id: r.sender.id,
            username: r.sender.username,
            tag: '0000',
            avatar: '/assets/avatars/default.png'
        }));
        const outgoingDetails = outgoing.map((r) => ({
            id: r.receiver.id,
...
router.get('/party', checkAuth, (req, res) => {
    const userId = req.userId;
...
router.post('/friends/request', checkAuth, async (req, res) => {
    const userId = req.userId;
    const { username } = req.body;
    try {
        const targetUser = await prisma_1.default.user.findUnique({ where: { username } });
        if (!targetUser) {
        ...
        const existing = await prisma_1.default.friendRequest.findFirst({
            where: {
                OR: [
        ...
        const alreadyFriends = await prisma_1.default.friendship.findFirst({
            where: { userId, friendId: targetUser.id }
        });
        if (alreadyFriends) {
            return res.status(400).json({ message: 'Already friends' });
        }
        await prisma_1.default.friendRequest.create({
            data: {
                senderId: userId,
...
router.post('/friends/accept', checkAuth, async (req, res) => {
    const userId = req.userId;
    const { requestId } = req.body;
    try {
        const senderId = parseInt(requestId);
        const request = await prisma_1.default.friendRequest.findFirst({
            where: {
                senderId: senderId,
                receiverId: userId,
                status: 'PENDING'
...
router.post('/party/create', checkAuth, async (req, res) => {
    const userId = req.userId;
    console.log(`[Social] Creating party for user ${userId}`);
    // Get username (optional, but good for display)
    const user = await prisma_1.default.user.findUnique({ where: { id: userId } });
    const username = user?.username || `User ${userId}`;
    const party = PartyManager_1.partyManager.createParty(userId, username);
    console.log(`[Social] Party created:`, party);
    res.json(party);
});
const socketHandler_1 = require("../sockets/socketHandler");
router.post('/party/invite', checkAuth, async (req, res) => {
    const userId = req.userId;
    ...
    const sent = (0, socketHandler_1.sendToUser)(io, targetUserId, 'party_invite', {
        partyId: party.id,
        senderId: userId,
        senderName: username
    });
...
router.post('/party/join', checkAuth, async (req, res) => {
    const userId = req.userId;
    const { partyId } = req.body;
    const user = await prisma_1.default.user.findUnique({ where: { id: userId } });
    const username = user?.username || `User ${userId}`;
    const party = PartyManager_1.partyManager.joinParty(partyId, userId, username);
    if (!party) {
    ...
    party.members.forEach(m => {
        (0, socketHandler_1.sendToUser)(io, m.userId, 'party_update', party);
    });
    res.json(party);
});
router.post('/party/leave', checkAuth, (req, res) => {
    const userId = req.userId;
    const party = PartyManager_1.partyManager.leaveParty(userId);
    if (party) {
...
router.post('/party/kick', checkAuth, (req, res) => {
    const userId = req.userId;
    const { userId: targetUserId } = req.body;
    const party = PartyManager_1.partyManager.getUserParty(userId);
    ...
    const updatedParty = PartyManager_1.partyManager.kickMember(party.id, targetUserId);
    const io = req.app.get('io');
    // Notify kicked user
    (0, socketHandler_1.sendToUser)(io, targetUserId, 'party_update', null); // null means no party
    (0, socketHandler_1.sendToUser)(io, targetUserId, 'party_kicked', { message: 'You were kicked from the party' });
    if (updatedParty) {
        // Notify remaining members
        updatedParty.members.forEach(m => {
            (0, socketHandler_1.sendToUser)(io, m.userId, 'party_update', updatedParty);
        });
    }
    res.json({ message: 'Player kicked' });
});
router.post('/party/ready', checkAuth, (req, res) => {
    const userId = req.userId;
    const party = PartyManager_1.partyManager.getUserParty(userId);
    if (!party) {
        return res.status(400).json({ message: 'Not in a party' });
    }
    const updatedParty = PartyManager_1.partyManager.toggleReady(party.id, userId);
    if (updatedParty) {
        const io = req.app.get('io');
        updatedParty.members.forEach(m => {
            (0, socketHandler_1.sendToUser)(io, m.userId, 'party_update', updatedParty);
